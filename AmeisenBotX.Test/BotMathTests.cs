...
namespace AmeisenBotX.Test
{
    ...
    public class BotMathTests
    {
        ...
        // [TestMethod] public void CalculatePositionAroundTest() { Vector3 middlePos = new
        // Vector3(0, 0, 0);
        //
        // Vector3 topPos = new Vector3(0, 4, 0); Vector3 leftPos = new Vector3(-4, 0, 0); Vector3
        // bottomPos = new Vector3(0, -4, 0); Vector3 rightPos = new Vector3(4, 0, 0);
        //
        // Assert.AreEqual(rightPos, BotMath.CalculatePositionAround(middlePos, 0f, 0f, 4));
        // Assert.AreEqual(topPos, BotMath.CalculatePositionAround(middlePos, 0f, (float)Math.PI /
        // 2f, 4)); Assert.AreEqual(leftPos, BotMath.CalculatePositionAround(middlePos, 0f,
        // (float)Math.PI, 4)); Assert.AreEqual(bottomPos,
        // BotMath.CalculatePositionAround(middlePos, 0f, (float)Math.PI * 1.5f, 4));
        // Assert.AreEqual(rightPos, BotMath.CalculatePositionAround(middlePos, 0f, (float)Math.PI *
        // 2f, 4)); }
        //
        // [TestMethod] public void CalculatePositionBehindTest() { Vector3 topPos = new Vector3(0,
        // 4, 0); Vector3 middlePos = new Vector3(0, 0, 0);
        //
        // float facingAngle = BotMath.GetFacingAngle2D(topPos, middlePos); Vector3 calculatedPos =
        // BotMath.CalculatePositionBehind(topPos, facingAngle, (float)2.0);
        //
        // Vector3 expectedPos = new Vector3(0, 6, 0); Assert.AreEqual(expectedPos, calculatedPos); }
