...
namespace AmeisenBotX.Core.Engines.Combat.Classes.Jannis
{
    public abstract class BasicCombatClass : SimpleConfigurable, ICombatClass
    {
        ...
        private bool CastSpell(string spellName, bool castOnSelf)
        {
            ...
            if (Bot.Wow.ExecuteLuaAndRead(BotUtils.ObfuscateLua($"{{v:3}},{{v:4}}=GetSpellCooldown(\"{spellName}\"){{v:2}}=(({{v:3}} or 0)+({{v:4}} or 0)-GetTime())*1000;if {{v:2}}<=0 then {{v:2}}=0;CastSpellByName(\"{spellName}\"{(castOnSelf ? ", \"player\"" : string.Empty)}){{v:5}},{{v:6}}=GetSpellCooldown(\"{spellName}\"){{v:1}}=(({{v:5}} or 0) + ({{v:6}} or 0) -GetTime())*1000;{{v:0}}=\"1;\"..{{v:1}} else {{v:0}}=\"0;\"..{{v:2}} end"), out string result))
            {
