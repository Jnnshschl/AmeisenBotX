...
namespace AmeisenBotX.Wow548.Objects
{
    public class ObjectManager : IObjectProvider
    {
        ...
        public ObjectManager(IMemoryApi memoryApi, IOffsetList offsetList)
        {
            ...
            Partymembers = new List<WowUnit548>();
            PartyPets = new List<WowUnit548>();
        }
        ...
        public void UpdateWowObjects()
        {
            lock (queryLock)
            {
                ...
                if (PlayerGuidIsVehicle)
                {
                    ...
                    WowPlayer548 lastKnownPlayer = wowObjects.OfType<WowPlayer548>().FirstOrDefault(e => e.Guid == Player.Guid);
                ...
                if (PartyleaderGuid > 0)
                {
                    ...
                    Partymembers = wowObjects.OfType<WowUnit548>().Where(e => PartymemberGuids.Contains(e.Guid));
                    ...
                    PartyPets = wowObjects.OfType<WowUnit548>().Where(e => PartymemberGuids.Contains(e.SummonedByGuid));
                }
        ...
        internal void HookManagerOnGameInfoPush(GameInfo gameInfo)
        {
            if (Player != null)
            {
                ((WowPlayer548)Player).IsOutdoors = gameInfo.isOutdoors;
            }
        ...
        private void ProcessObject(int i)
        {
                ...
                && MemoryApi.Read(IntPtr.Add(ptr, (int)OffsetList.WowObjectDescriptor), out IntPtr descriptorAddress))
            {
                ...
                {
                    WowObjectType.Container => new WowContainer548(ptr, descriptorAddress),
                    WowObjectType.Corpse => new WowCorpse548(ptr, descriptorAddress),
                    WowObjectType.Item => new WowItem548(ptr, descriptorAddress),
                    WowObjectType.DynamicObject => new WowDynobject548(ptr, descriptorAddress),
                    WowObjectType.GameObject => new WowGameobject548(ptr, descriptorAddress),
                    WowObjectType.Player => new WowPlayer548(ptr, descriptorAddress),
                    WowObjectType.Unit => new WowUnit548(ptr, descriptorAddress),
                    _ => new WowObject548(ptr, descriptorAddress)
                };
                ...
                if (wowObjects[i] != null)
                {
                    ...
                    if (type is WowObjectType.Unit or WowObjectType.Player)
                    {
                        ...
                        {
                            PlayerGuidIsVehicle = wowObjects[i] is not WowPlayer548;

                            if (!PlayerGuidIsVehicle)
                            {
                                Player = (WowPlayer548)wowObjects[i];
                                Vehicle = null;
                            }
                            else
                            {
                                // player stays the old object
                                Vehicle = (WowUnit548)wowObjects[i];
                            }
                        }

                        if (wowObjects[i].Guid == TargetGuid) { Target = (WowUnit548)wowObjects[i]; }
                        if (wowObjects[i].Guid == LastTargetGuid) { LastTarget = (WowUnit548)wowObjects[i]; }
                        if (wowObjects[i].Guid == PartyleaderGuid) { Partyleader = (WowUnit548)wowObjects[i]; }
                        if (wowObjects[i].Guid == PetGuid) { Pet = (WowUnit548)wowObjects[i]; }
                    }
                }
            }
        }
